class Solution {
  ...
  public int[] minOperationsQueries(int n, int[][] edges, int[][] queries) {
    ...
    wtFreq = new int[n][27];// from root to a particular node what is the freq of occurence of a pariticular
                            // weight value
    ...
    for (int q = 0; q < queries.length; q++) {
      ...
      // total number of weightValues occruring between vertex u and v minus the freq
      // of weightValue occuring maximum number of times between vertex or say node u
      // and v is the answer for the current query
  ...
  private int LcaUsingBinaryLifting(int u, int v) {
    ...
    // bring the nodes at the same level, for that start form the more deep node and
    // going their depthLevel difference upward
    ...
    for (int bitPos = 30; bitPos >= 0; bitPos--) {
      // if there comes a point when there ancesstors are not equal move u and v to
      // their ancestors
  ...
  // populateData is used to populate the depthLevel, parentOf, and wtFreq arrays
  // or say vector which will help us to calculate LcaUsingBinaryLifting and
  // answer the queries
  ...
  private int[][] buildparentOfMatrix() {
    ...
    for (int bitPos = 1; bitPos < limit; bitPos++) {
      for (int indx = 0; indx < n; indx++) {
        ...
        // currNode----->((2^(bitPos-1)) upward from
        // currNode)(prevParent)----------->(2^(bitPos-1)) upward form
        // prevParent-------->target(parMat[bitPos][indx])((2^(bitPos)) upward from)
